...
class KinisiCommands:
    ...
    def initialize_motor(self, motorIndex:MotorIndex):
        msg = INITIALIZE_MOTOR.to_bytes(1, 'little') + motorIndex.to_bytes(1, 'little')
        length = len(msg)
        msg = length.to_bytes(1, 'little') + msg
        self.write(msg)

    # This command sets the speed of the specified motor.
    def set_motor_speed(self, motorIndex:MotorIndex, direction:int, speed:int):
        msg = SET_MOTOR_SPEED.to_bytes(1, 'little') + motorIndex.to_bytes(1, 'little') + direction.to_bytes(1, 'little') + speed.to_bytes(2, 'little')
        length = len(msg)
        msg = length.to_bytes(1, 'little') + msg
        self.write(msg)

    # This command sets the controller for the specified motor.
    def initialize_motor_controller(self, motorIndex:MotorIndex, kp:float, ki:float, kd:float):
        msg = INITIALIZE_MOTOR_CONTROLLER.to_bytes(1, 'little') + motorIndex.to_bytes(1, 'little') + bytearray(struct.pack('d', kp)) + bytearray(struct.pack('d', ki)) + bytearray(struct.pack('d', kd))
        length = len(msg)
        msg = length.to_bytes(1, 'little') + msg
        self.write(msg)
    ...
    def set_motor_target_velocity(self, motorIndex:MotorIndex, direction:int, speed:int):
        msg = SET_MOTOR_TARGET_VELOCITY.to_bytes(1, 'little') + motorIndex.to_bytes(1, 'little') + direction.to_bytes(1, 'little') + speed.to_bytes(2, 'little')
        length = len(msg)
        msg = length.to_bytes(1, 'little') + msg
        self.write(msg)

    # This command deletes the controller for the specified motor.
    def delete_motor_controller(self, motorIndex:MotorIndex):
        msg = DELETE_MOTOR_CONTROLLER.to_bytes(1, 'little') + motorIndex.to_bytes(1, 'little')
        length = len(msg)
        msg = length.to_bytes(1, 'little') + msg
        self.write(msg)

    # This command initializes an encoder and prepares it for use.
    def initialize_encoder(self, encoderIndex:EncoderIndex):
        msg = INITIALIZE_ENCODER.to_bytes(1, 'little') + encoderIndex.to_bytes(1, 'little')
        length = len(msg)
        msg = length.to_bytes(1, 'little') + msg
        self.write(msg)

    # This command retrieves the current value of the encoder.
    def get_encoder_value(self, encoderIndex:EncoderIndex):
        msg = GET_ENCODER_VALUE.to_bytes(1, 'little') + encoderIndex.to_bytes(1, 'little')
        length = len(msg)
        msg = length.to_bytes(1, 'little') + msg
        self.write(msg)
        return self.read(4)
    ...
    def set_pin_low(self, pinNumber:int):
        msg = SET_PIN_LOW.to_bytes(1, 'little') + pinNumber.to_bytes(1, 'little')
        length = len(msg)
        msg = length.to_bytes(1, 'little') + msg
        self.write(msg)

    # This command sets the specified pin to a high state.
    def set_pin_high(self, pinNumber:int):
        msg = SET_PIN_HIGH.to_bytes(1, 'little') + pinNumber.to_bytes(1, 'little')
        length = len(msg)
        msg = length.to_bytes(1, 'little') + msg
        self.write(msg)

    # This command toggles the specified pin.
    def toggle_pin(self, pinNumber:int):
        msg = TOGGLE_PIN.to_bytes(1, 'little') + pinNumber.to_bytes(1, 'little')
        length = len(msg)
        msg = length.to_bytes(1, 'little') + msg
        self.write(msg)
    ...
    def set_platform_velocity_input(self, x:int, y:int, t:int):
        msg = SET_PLATFORM_VELOCITY_INPUT.to_bytes(1, 'little') + x.to_bytes(1, 'little') + y.to_bytes(1, 'little') + t.to_bytes(1, 'little')
        length = len(msg)
        msg = length.to_bytes(1, 'little') + msg
        self.write(msg)
